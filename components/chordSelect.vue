<template>
  <div class="container-chordSelect">
    <div class="key-select">
      <div v-for="(chord, i) in chordTone" :key="'reactkey_' + i" :id="'key' + (i + 1)" class="key-num square" 
        ...
        <div :class="'key-bar' + ('keyBarShow'+(i+1) ? ' keybar-show' : '')" :style="{left : 'keyBar'+(i+1)+'px'}"></div>
      </div>
    </div>
    <div class="select-notify noselect">左右拖动可改变和弦组成音</div>
    <div class="chord-count">
      <div :class="type === 3 ? 'noselect active' : 'noselect'" @click="chordCountChange(3)">三音和弦</div>
      <div :class="type === 4 ? 'noselect active' : 'noselect'" @click="chordCountChange(4)">四音和弦</div>
    </div>
    <div :class="'loading-box' + (loading?' show' : '')">
...
export default {
  ...
  methods: {
    ...
    touchMove(index, e) {
      ...
      _state.chordTone = this.chordTone.concat()
      _state.chordTone[index] = newKey
      this.$set('data', _state)
    },
    touchStart(index, e) {
      let keyBarName = "keyBar" + (index + 1)
      let _state = {}
    ...
    touchEnd(index) {
      ...
      _state.loading = true
      this.$set('data', _state)
      this.selectFinish(this.chordTone)
    },
    mouseDown(index, e) {
    ...
    chordCountChange(count) {
      this.chordTone = this.chordTone.slice(0, count),
      this.type = count
      if (count === 4 && this.chordTone.length < 4) {
        this.chordTone = this.chordTone.concat(['1']),
        this.keyBar4 = 0
      }
    },
    hideLoading() {
